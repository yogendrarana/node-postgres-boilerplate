...
import * as authSchema from '../zod/auth.js';
import ErrorHandler from "../util/errorHandler.js";
import { asyncHandler } from "../util/asyncHandler.js";
import * as tokenService from '../services/token.js';
import { userSchema } from '../config/db/schema/user.js';
import { refreshTokenSchema } from '../config/db/schema/token.js';
// registerUser
export const registerUser = asyncHandler(async (req, res, next) => {
    const { name, email, password, confirm_password } = req.body;
    // check if email and password are provided
    if (!email || !password) {
        return next(new ErrorHandler(401, "Please provide an email and password."));
    }
